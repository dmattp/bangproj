...
class BangPsObject : public Bang::Function
{
    gcroot<PSObject^> psobject_;
public:
    BangPsObject( PSObject^ o ) // : psobject_(o)
    {
        psobject_ = o;
    }

    void push_property( Bang::Stack& s, const std::string& propname );
...
//            ||  Object::ReferenceEquals(property->Value->GetType(),System::ServiceProcess::ServiceControllerStatus::typeid)
            ...
            )
            {
                ...
                if (0)
                {}
                else if ( Object::ReferenceEquals(property->Value->GetType(),System::String::typeid) )
                {
                    std::string propValue;
                    MarshalString( safe_cast<System::String^>(property->Value), propValue );
                    s.push( propValue ); // tResult[propName] = propValue;
                }
...
void BangPsObject::push_property( Bang::Stack& s, const std::string& propname )
{
    PSMemberInfoCollection<PSPropertyInfo^>^ properties = psobject_->Properties;
    ...
    Console::WriteLine(
        ...
        result->Members["HandleCount"]->Value);
#endif             
    String^ netpropname = gcnew String(propname.c_str());
    //for each (PSPropertyInfo^ property in properties)
    {
        PushBangFromNetProperty( s, properties[netpropname] ) ; // , bAllowMoreRecursion );
    }
}
...
public:
    ...
    void invoke( Bang::Stack& s, const RunContext& rc, const std::string& cmd )
    {
...
public:
    ...
    void invoke( Bang::Stack& s, const RunContext& rc, const std::string& cmd )
    {
        return pshell_->invoke( s, rc, cmd );
    }
//     void invoke_threadreporter( lua_State* l, const std::string& cmd, ThreadReporter reporter )
//     {
...
void PshellUnmanWrap::invoke( Bang::Stack&s, const RunContext& rc, const std::string& cmd )
{
    return pimpl_->invoke(s, rc, cmd);
}
